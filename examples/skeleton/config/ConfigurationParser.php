...
class ConfigurationParser {
    ...
    public function Run() {
        if(is_null($this->Configuration)) {
            ...
            $ConfigurationContent = preg_replace_callback('/^(|\s)%%INCLUDE (.*?)$/im', function ($match) use ($dir) {
                if(file_exists($dir.$match[2]))
                    return "\n".file_get_contents($dir.$match[2])."\n";
        ...
        if(is_scalar($value)) {
            ...
            if(preg_match_all('/%%CONSTANT_([A-Z0-9_]{1,})/', $value, $matches)) {
                foreach($matches[1] AS $constant) {
                    if(defined($constant)) {
                        $value = preg_replace('/%%CONSTANT_' . $constant . '/', constant($constant), $value);
                    }
                }
            }
        } else {
